            ...
            if(msgUsrInput.trim().length>0&&passUsrInput.trim().length>0){
                if(msgUsrInput!=passUsrInput){
                    ...
                    if(msgUsrInput=="60160027"||msgUsrInput=="60160099"||msgUsrInput=="60160164"||msgUsrInput=="60160167"
                        ...
                        val fragment_menu_music = Menu_music()
                        val fm = fragmentManager
                        val transaction : FragmentTransaction = fm!!.beginTransaction()
                        transaction.replace(R.id.contentContainer, fragment_menu_music,"fragment_list_view")
    ...
    private fun handleFacebookAccessToken(token : AccessToken) {
        ...
        firebaseAuth!!.signInWithCredential(credential).addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                val Menu_music = Menu_music().newInstance(user!!.photoUrl.toString(),user!!.displayName.toString())
                val fm = fragmentManager
                val transaction : FragmentTransaction = fm!!.beginTransaction()
                transaction.replace(R.id.contentContainer, Menu_music,"fragment_menu_music")
                transaction.addToBackStack("fragment_menu_music")
                transaction.commit()
